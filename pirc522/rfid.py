...
# 3 to 0 reserved - reserved for future use
	...
	def __init__(self, bus=0, device=0, speed=1000000, pin_rst=def_pin_rst,
			pin_ce=0, pin_irq=def_pin_irq, pin_mode = def_pin_mode, default_timeout = def_default_timeout):
	...
	def wait_for_tag(self, timeout = None):
		...
		# wait for it
		waiting = True
		while waiting and (not time.time()-now >= timeout):
			self.init()
			#self.irq.clear()
			self.dev_write(0x04, 0x00)
			self.dev_write(0x02, 0xA0)
	...
	def wait_for_tag_uid(self, timeout = None):
		...
		while (not time.time()-now >= timeout):
			self.wait_for_tag(timeout = timeout)
